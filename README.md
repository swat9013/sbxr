...
- VM が sbxr の外で撤去されていて状態ディレクトリだけが残っている（VM 消失）ときは、差分を比べずに `sbxr destroy` を促して非 0 で終える。`sbxr stop` も同じく止まる。`sbxr destroy` は VM が無くても sandbox スコープの secret と状態ディレクトリを片付ける
